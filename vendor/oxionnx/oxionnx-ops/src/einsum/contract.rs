// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS an N-split parallel path for single-batch contractions.
...
fn gemm_batched(
    ...
    #[cfg(any(not(target_arch = "wasm32"), feature = "wasm-threads"))]
    {
        ...
        // Single batch: split N across threads, as the conv GEMM path does
        // (`conv2d::parallel_sgemm`). The FFC Fourier-unit contractions are
        // `m=64, k=64, n=6336|12288` and were running single-threaded at
        // ~72 GFLOP/s; the same shapes reach ~400 GFLOP/s split 16 ways.
        //
        // N-split rather than M-split because `matrixmultiply` packs B on
        // every call: an M-split would have each thread pack the whole
        // `[k, n]` operand independently, while an N-split gives every thread
        // a disjoint column range of B and C so B is packed once in total.
        // Bit-identical: the K-accumulation order of each output element does
        // not depend on how N is split.
        if batch == 1 && flops >= PARALLEL_GEMM_FLOPS {
            use rayon::prelude::*;
            let threads = rayon::current_num_threads().max(1);
            if n >= threads * 32 {
                // The FFC contractions are `m=64, k=64` — tiny. Their
                // per-call packing cost is fixed while the useful work
                // shrinks with the chunk width, so a chunk budget is not
                // the same as a thread budget: fewer, wider chunks can beat
                // one chunk per thread. Measured on `m=64, k=64, n=6336`:
                // 4 chunks → 402 ms, 8 → 379 ms, 16 (one per thread) → 364 ms
                // node total, so default to one chunk per thread.
                let chunks = threads;
                let chunk = n.div_ceil(chunks).max(1);
                let b_ptr = b.as_ptr() as usize;
                let c_ptr = out.as_mut_ptr() as usize;
                (0..n).into_par_iter().step_by(chunk).for_each(|n0| {
                    let bn = chunk.min(n - n0);
                    // SAFETY: each task owns the disjoint column range
                    // `[n0, n0 + bn)` of B (read) and C (write). The slices
                    // span the strided access up to its last touched element,
                    // which lies inside the parent buffers:
                    // `(k-1)*n + n0 + bn <= k*n` and `(m-1)*n + n0 + bn <= m*n`.
                    unsafe {
