// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS a scalar-divisor SIMD path to div().
...
pub fn div(a: &Tensor, b: &Tensor) -> Result<Tensor, String> {
    #[cfg(feature = "simd")]
    {
        ...
        // Scalar divisor: `Div(x, const)` is the shape every `Div` node in
        // the LaMa FFC graph has (the rttn normalisation). The generic
        // broadcast walk costs ~5-7 cycles per element; a broadcast `vdivps`
        // is IEEE-exact and replaces it element-for-element.
        if b.numel() == 1 && !a.data.is_empty() {
            let d = b.data[0];
            let mut out = vec![0.0f32; a.data.len()];
            crate::simd_ops::simd_div_scalar(&a.data, d, &mut out);
            return Ok(Tensor::new(out, a.shape.clone()));
        }
    }
    elementwise_binary(a, b, |x, y| x / y)
