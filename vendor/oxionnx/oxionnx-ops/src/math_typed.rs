// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS merged-GEMM broadcast folding to matmul_f32_into.
...
pub(crate) fn matmul_f32_into(
    ...
    // ── Merged-GEMM fast paths ───────────────────────────────────────────────
    //
    // The naive loop calls `sgemm` once per batch element. For the FFC
    // Fourier-unit shapes (batch=6336, m=k=64, n=1) that is ~6336 tiny
    // calls per node, and `matrixmultiply`'s per-call packing overhead
    // dwarfs the 8192 FLOP of actual work — ~1 second per inference on
    // LaMa. Both broadcast patterns collapse to a single dense GEMM:

    // Case 1: B is shared across all batches (b_batches == 1).
    // Stack A along M: [batch*m, k] @ [k, n] = [batch*m, n], which is
    // exactly `out` in row-major order. Contiguity holds because A's
    // batches are contiguous [m, k] blocks when a_batches == batch_size.
    if b_batches == 1 && a_batches == batch_size && batch_size > 1 {
        sgemm_strided(
            batch_size * m,
            k,
            n,
    ...
    // Case 2: A is shared across all batches (a_batches == 1) and n == 1.
    // out[b] = A @ B[b] means out[b*m + i] = Σ_c A[i,c] * B[b,c] =
    // Σ_c B[b,c] * A^T[c,i]. Computed as C' = B @ A^T with the batch dim
    // on M: C'[batch, m] row-major lands directly in `out` (contiguous
    // writes), B is read row-major [batch, k] (contiguous reads), and A^T
    // is the small (m×k) shared operand read with swapped strides.
    if a_batches == 1 && n == 1 && b_batches == batch_size && batch_size > 1 {
        sgemm_strided(
            batch_size,
            k,
            m,
    ...
    // Case 3: both operands shared across all batches.
    // One GEMM, then replicate the result into every batch slot.
    if a_batches == 1 && b_batches == 1 && batch_size > 1 {
        sgemm_strided(
            m,
            k,
            n,
