// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS a single-pass SIMD batch-norm channel kernel.
...
pub(crate) fn batch_norm_into(
    ...
    // One SIMD pass over every channel plane; the scalar fallback reproduces
    // `((x - m) * inv_std) * scale + bias` element-for-element.
    for ni in 0..n {
        for ci in 0..c {
            let s = scale.data[ci];
            let b = bias.data[ci];
            ...
            let xplane = &x.data[base..base + spatial];
            let oplane = &mut out[base..base + spatial];
            #[cfg(feature = "simd")]
            {
                crate::simd_ops::simd_batch_norm_channel(xplane, oplane, m, inv_std, s, b);
            }
            #[cfg(not(feature = "simd"))]
            {
                for (o, &vx) in oplane.iter_mut().zip(xplane.iter()) {
                    *o = ((vx - m) * inv_std) * s + b;
                }
            }
        }
