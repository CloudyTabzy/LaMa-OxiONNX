// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS div_scalar and batch_norm_channel AVX2 kernels.
...
pub(super) mod avx2_impl {
    ...
    /// Caller must ensure AVX2 is supported.
    #[target_feature(enable = "avx2")]
    pub unsafe fn batch_norm_channel(
        x: &[f32],
        out: &mut [f32],
        mean: f32,
        inv_std: f32,
        ...
        for i in 0..chunks {
            ...
            // Same operation order as the scalar loop: ((x - m) * inv) * s + b.
            let t = _mm256_mul_ps(_mm256_mul_ps(_mm256_sub_ps(vx, vm), vi), vs);
            _mm256_storeu_ps(out.as_mut_ptr().add(o), _mm256_add_ps(t, vb));
        }
        for i in (chunks * LANE_WIDTH)..n {
    ...
    /// Caller must ensure AVX2 is supported.
    #[target_feature(enable = "avx2")]
    pub unsafe fn div_scalar(a: &[f32], d: f32, out: &mut [f32]) {
        let n = a.len();
        let chunks = n / LANE_WIDTH;
        ...
        for i in 0..chunks {
            let offset = i * LANE_WIDTH;
            let va = _mm256_loadu_ps(a.as_ptr().add(offset));
            _mm256_storeu_ps(out.as_mut_ptr().add(offset), _mm256_div_ps(va, vd));
        }
        for i in (chunks * LANE_WIDTH)..n {
            out[i] = a[i] / d;
        }
