// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS SIMD wrappers for the new kernels.
...
/// SIMD-accelerated per-channel batch-norm transform over one contiguous
/// channel plane: `out[i] = ((x[i] - mean) * inv_std) * scale + bias`.
///
/// The scalar loop this replaces ran at ~2-3 cycles per element over every
/// element of every BatchNormalization node (the FFC decoder has 75 of them),
/// plus a full `copy_from_slice` pass it no longer needs. Operation order is
/// unchanged, so the result is bit-identical.
pub fn simd_batch_norm_channel(
    x: &[f32],
    out: &mut [f32],
    mean: f32,
    inv_std: f32,
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            ...
            unsafe {
                super::avx2::avx2_impl::batch_norm_channel(x, out, mean, inv_std, scale, bias);
            }
            return;
        }
    }
    for i in 0..x.len() {
        out[i] = ((x[i] - mean) * inv_std) * scale + bias;
    }
}

/// SIMD-accelerated element-wise division by a scalar: `out[i] = a[i] / d`.
///
/// Uses IEEE `vdivps` per element rather than multiplying by a precomputed
/// reciprocal, so the result is bit-identical to the scalar `a[i] / d` loop
/// it replaces — the ONNX `Div(x, scalar)` nodes in real graphs (the FFC
/// Fourier-unit normalisation, for one) hit the scalar broadcast path inside
/// `elementwise_binary` at roughly 5-7 cycles per element otherwise.
pub fn simd_div_scalar(a: &[f32], d: f32, out: &mut [f32]) {
    debug_assert_eq!(a.len(), out.len());
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            ...
            AVX2_DISPATCH_HITS.with(|c| c.set(c.get() + 1));
            unsafe {
                super::avx2::avx2_impl::div_scalar(a, d, out);
            }
            return;
        }
    }
