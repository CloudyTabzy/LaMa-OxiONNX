// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): ADDS unit tests for fuse_add_batchnorm.
...
use super::{fold_batch_norm_inference, fuse_add_batchnorm, fuse_conv_add_relu, fuse_conv_batchnorm};
...
#[test]
fn test_fuse_add_batchnorm_folds_both_branches() {
    let conv_a = make_node(OpKind::Conv, "conv_a", vec!["x", "wa"], vec!["a_out"]);
    let conv_b = make_node(OpKind::Conv, "conv_b", vec!["g", "wb"], vec!["b_out"]);
    let add = make_node(OpKind::Add, "add", vec!["a_out", "b_out"], vec!["add_out"]);
    let mut bn = make_node(
        OpKind::BatchNorm,
        "bn",
        vec!["add_out", "bn_scale", "bn_bias", "bn_mean", "bn_var"],
        vec!["bn_out"],
    );
    bn.attrs.floats.insert("epsilon".to_string(), 1e-5);

    let nodes = vec![conv_a, conv_b, add, bn];
    let mut weights = HashMap::new();
    weights.insert("wa".to_string(), Tensor::new(vec![2.0], vec![1, 1, 1, 1]));
    weights.insert("wb".to_string(), Tensor::new(vec![3.0], vec![1, 1, 1, 1]));
    weights.insert("bn_scale".to_string(), Tensor::new(vec![4.0], vec![1]));
    weights.insert("bn_bias".to_string(), Tensor::new(vec![0.5], vec![1]));
    weights.insert("bn_mean".to_string(), Tensor::new(vec![1.0], vec![1]));
    weights.insert("bn_var".to_string(), Tensor::new(vec![4.0], vec![1]));

    let result = fuse_add_batchnorm(nodes, &mut weights, &["bn_out".to_string()]);

    // The BatchNorm is gone; the Add now produces its output.
    assert_eq!(result.len(), 3);
    let add = result.iter().find(|n| matches!(n.op, OpKind::Add)).expect("add");
    assert_eq!(add.outputs, vec!["bn_out".to_string()]);
    assert!(result.iter().all(|n| !matches!(n.op, OpKind::BatchNorm)));

    let k = 4.0f32 / (4.0f32 + 1e-5).sqrt();
    let wa = weights.get("a_out_addbn_weight").expect("fused wa");
    assert!((wa.data[0] - 2.0 * k).abs() < 1e-5);
    let ba = weights.get("a_out_addbn_bias").expect("fused bias a");
    // First branch carries the whole shift: (bias_A - mean) * k + bn_bias.
    let expected_ba = (0.0 - 1.0) * k + 0.5;
    assert!((ba.data[0] - expected_ba).abs() < 1e-5);

    let wb = weights.get("b_out_addbn_weight").expect("fused wb");
    assert!((wb.data[0] - 3.0 * k).abs() < 1e-5);
    let bb = weights.get("b_out_addbn_bias").expect("fused bias b");
    assert!(bb.data[0].abs() < 1e-6);

    // a' + b' == BN(a + b) for an arbitrary input.
    let x = 1.75f32;
    let a = 2.0 * x;
    let b = 3.0 * x;
    let original = (a + b - 1.0) * k + 0.5;
    let folded = (2.0 * k * x + expected_ba) + (3.0 * k * x + 0.0);
    assert!((original - folded).abs() < 1e-5);
}

#[test]
fn test_fuse_add_batchnorm_declines_shared_conv_output() {
    let conv_a = make_node(OpKind::Conv, "conv_a", vec!["x", "wa"], vec!["a_out"]);
    let conv_b = make_node(OpKind::Conv, "conv_b", vec!["g", "wb"], vec!["b_out"]);
    let add = make_node(OpKind::Add, "add", vec!["a_out", "b_out"], vec!["add_out"]);
    let bn = make_node(
    ...
    // A second consumer of conv_b's output blocks the fold.
    let relu = make_node(OpKind::Relu, "relu", vec!["b_out"], vec!["relu_out"]);

    let nodes = vec![conv_a, conv_b, add, bn, relu];
    let mut weights = HashMap::new();
    weights.insert("wa".to_string(), Tensor::new(vec![2.0], vec![1, 1, 1, 1]));
    weights.insert("wb".to_string(), Tensor::new(vec![3.0], vec![1, 1, 1, 1]));
    weights.insert("bn_scale".to_string(), Tensor::new(vec![4.0], vec![1]));
    ...
    assert!(result.iter().any(|n| matches!(n.op, OpKind::BatchNorm)));
    assert!(!weights.contains_key("b_out_addbn_weight"));
}

#[test]
fn test_fuse_add_batchnorm_declines_graph_output_add() {
    let conv_a = make_node(OpKind::Conv, "conv_a", vec!["x", "wa"], vec!["a_out"]);
    let conv_b = make_node(OpKind::Conv, "conv_b", vec!["g", "wb"], vec!["b_out"]);
    let add = make_node(OpKind::Add, "add", vec!["a_out", "b_out"], vec!["add_out"]);
    let bn = make_node(
    ...
    // `add_out` is a declared graph output, so it must keep being produced:
    // the fold would rename it away and is declined.
    let result = fuse_add_batchnorm(
        nodes,
        &mut weights,
        &["add_out".to_string(), "bn_out".to_string()],
    );
    assert_eq!(result.len(), 4);
    assert!(result.iter().any(|n| matches!(n.op, OpKind::BatchNorm)));
}
