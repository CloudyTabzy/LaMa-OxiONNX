// Modified by the GIMP LaMa inpainting fork of OxiONNX (2026-09): wires the fuse_add_batchnorm pass and its OXIONNX_NO_ADDBN_FUSION kill switch.
...
pub fn optimize_with_input_shapes(
    ...
    // FFC-style blocks put a residual Add of two Convs under the BatchNorm,
    // which `fuse_conv_batchnorm` cannot see through; this pass folds that
    // shape by distributing the normalisation into both branches.
    //
    // `OXIONNX_NO_ADDBN_FUSION=1` disables it, so the fold's effect can be
    // measured A/B in one binary (the session cache must be bypassed for the
    // two variants to describe different graphs — the worker honours
    // `OXIONNX_NO_SESSION_CACHE=1` for that).
    #[cfg(not(target_arch = "wasm32"))]
    let add_bn_fusion = std::env::var_os("OXIONNX_NO_ADDBN_FUSION").is_none();
    #[cfg(target_arch = "wasm32")]
    let add_bn_fusion = true;
    let nodes = if add_bn_fusion {
        fusion::fuse_add_batchnorm(nodes, weights, output_names)
    } else {
        nodes
    };
